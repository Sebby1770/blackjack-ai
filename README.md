...
them — plus a **Hi-Lo card counter** that turns the house edge positive. Four
...
`./blackjack compare --episodes 1000000 --hands 10000000`.
...
- Starting from zero knowledge, both RL agents converge to within a fraction of a
...
```bash
# Train every agent and benchmark them (incl. the card counter):
./blackjack compare --episodes 1000000 --hands 10000000
...
./blackjack train q --episodes 500000 --out q.policy --chart
...
│   ├── QLearningAgent, MonteCarloAgent, BasicStrategyAgent,
│   │   CountingAgent, RandomAgent  – the agents
│   ├── Chart                       – render a policy as a strategy grid
...
shoe and its Hi-Lo count, environment + rule invariants, basic-strategy and
counting decisions, strategy-chart generation, save/load round trips, and a
learning check that both RL agents beat the random baseline. It runs on every
push via **GitHub Actions** on Linux and macOS.
